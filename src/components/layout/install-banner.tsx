  ...
  return (
    <div className="flex items-center gap-3 mb-3 p-3 rounded-2xl bg-brand-50 dark:bg-brand-900/20 border border-brand-100 dark:border-brand-900/50">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-brand-700 dark:text-brand-300">{t('app.install')}</p>
        <p className="text-xs text-brand-600/80 dark:text-brand-400/80">{t('app.installHint')}</p>
      </div>
      <button
        type="button"
      ...
      <button type="button" onClick={dismiss} aria-label={t('app.installDismiss')} className="p-1 rounded-lg text-brand-500 hover:bg-brand-100 dark:hover:bg-brand-900/40">
        <X size={16} />
      </button>
    </div>
  );
