...
/** Reads the language from context when it is available; falls back to English otherwise. */
function ErrorFallback({ message }: { message: string }) {
  const { t } = useT();
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
      <p className="text-lg font-bold mb-1">{t('app.errorTitle')}</p>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">{t('app.errorBody')}</p>
      ...
      <p className="text-[10px] text-gray-400 mt-6 max-w-xs break-words">{message}</p>
    </div>
  );
}
