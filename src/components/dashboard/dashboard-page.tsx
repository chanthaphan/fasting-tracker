  ...
  return (
    ...
    >
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl p-4 mb-3 border border-gray-100 dark:border-gray-800">
        ...
        <SodiumBar total={totals.sodium} goal={sodiumGoalOf(goals)} label="Sodium" unit="mg" overLabel="Over goal" />
        {/* Net calories line */}
        {todayExercise > 0 && (
...
/** Sodium against the daily limit; turns red past it because more is the problem, not less. */
export function SodiumBar({ total, goal, label, unit, overLabel }: { total: number; goal: number; label: string; unit: string; overLabel: string }) {
  ...
  return (
    <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-800">
      <div className="flex flex-wrap items-center justify-between gap-x-3 text-xs text-gray-500 dark:text-gray-400">
        <span>{label}: <b className={over ? 'text-red-500' : 'text-teal-600 dark:text-teal-400'}>{total.toLocaleString()}</b> / {goal.toLocaleString()} {unit}</span>
      ...
      <div className="w-full h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full mt-1 overflow-hidden">
        <div className={`h-full rounded-full transition-all duration-500 ${over ? 'bg-red-500' : 'bg-teal-500'}`} style={{ width: `${Math.min((total / Math.max(goal, 1)) * 100, 100)}%` }} />
      </div>
    </div>
  );
}
