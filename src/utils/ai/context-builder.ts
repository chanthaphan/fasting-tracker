...
export function buildHealthContext(state: AppState, now: Date = new Date()): string {
  ...
  lines.push(
    `Daily goals: ${state.goals.calories} kcal, ${state.goals.protein}g protein, ${state.goals.carbs}g carbs, ${state.goals.fat}g fat, ${sodiumGoalOf(state.goals)} mg sodium max`
  );
  if (state.weightGoal) {
  ...
  for (let i = 0; i < FOOD_DAYS; i++) {
    ...
    dayLines.push(
      `${day}: ${t.calories} kcal (P${Math.round(t.protein)} C${Math.round(t.carbs)} F${Math.round(t.fat)}${t.sodium > 0 ? ` Na${Math.round(t.sodium)}mg` : ''})${burned > 0 ? `, exercise -${burned} kcal` : ''}`
    );
  }
